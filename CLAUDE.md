...
- **Installable as a home-screen/dock app** — `public/manifest.json` + `public/icon*.png` (cropped/resized from `public/branding.jpeg`, the source brand photo — regenerate with Pillow if it ever changes, `sips` can resize but not re-crop precisely), linked from both `AppLayout.astro` and `login.astro`'s `<head>` (no shared layout between them, so the tags are duplicated by hand). No service worker — this is an always-online internal tool, so it's install-for-appearance only (own icon, no browser chrome), not offline support.
