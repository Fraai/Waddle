...
- **Favorites** — any non-Inbox project can be starred (`projects.is_favorite` + its own `favorite_position`, independently drag-reorderable from the main Projects list) to also show in a "Favorites" sidebar section above Projects. It's a second lens, not a move — a favorited project still shows in the regular list too, star filled there as well. The Favorites rows are deliberately minimal (dot, name, star only — no rename/delete/colour-change), since the existing rename/delete/colour handlers only update the one row they're clicked in; letting a project be fully edited from two places at once would leave whichever row wasn't clicked stale until reload. Toggling favorite (from either location) just reloads on success rather than trying to keep both lists in sync live. Inbox is exempt, same reasoning as `setProjectType`.
- **Nested projects, one level only** — `projects.parent_project_id`, enforced in `lib/db.ts` (not a DB constraint, SQLite can't express "no grandchildren") by `setProjectParent`/`assertEligibleParent`: a candidate parent can't itself have a parent, and a project with children of its own can't become a child. `position` means "order among siblings" (same `parent_project_id`, NULL counts as its own group), not a single global sequence. Deleting a project promotes its children to top-level rather than cascading. The sidebar renders one flat `#project-list`, tree-ordered (each parent immediately followed by its own children, indented via `.project-row--child`) rather than nested `<ul>`s — simpler, and the existing Sortable/`reorderProjects` wiring keeps working with just an `onMove` guard (`sidebar.ts`) comparing `data-parent-id` to stop a drag crossing into a different sibling group. Reparenting is a `<select>` per row (only rendered on projects without children of their own, since those structurally can't become a child), not drag-and-drop; changing it reloads, same reasoning as Favorites. A project's own current parent is always kept as an option in its own select even once that parent gains other children too (see the comment in `AppLayout.astro` — this was a real bug: `eligibleParents` alone excludes it).
